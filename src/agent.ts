...
export class TellboyAgent extends Think<Env> {
  ...
  getSystemPrompt(): string {
    ...
    return [
      ...
      "Format replies with Markdown — it renders as native Telegram Rich Messages, so the full GitHub-Flavored Markdown vocabulary is available and displays properly: **bold**, `inline code`, ```fenced code blocks```, bullet/numbered lists (including nested), > blockquotes, ||spoilers||, # / ## headings to structure longer answers, GFM | tables | for tabular or comparative data, and --- dividers between sections. Reach for headings, tables and nested lists when the content genuinely benefits (comparisons, structured breakdowns, step-by-step with sub-points); keep short answers plain. Match the formatting to the content — don't over-format a one-line reply.",
